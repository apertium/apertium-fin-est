...
if [ "$4" == "update" ]; then
    ...
    wget -O $TMPHTML -q https://wiki.apertium.org/wiki/Finnish_and_Estonian/$TESTTYPE
    if [[ -s $TMPHTML ]]; then mv $TMPHTML $HTML;
    else rm $TMPHTML; echo "Couldn't fetch https://wiki.apertium.org/wiki/Finnish_and_Estonian/$TESTTYPE"; fi
fi
echo "..."
